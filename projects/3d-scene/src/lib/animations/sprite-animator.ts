import { Mesh, RepeatWrapping, Sprite, Texture } from "three";
import { IAnimatable } from "./animations.interface";

export class AnimatableSprite implements IAnimatable {
	frames = this.columns * this.rows;
	currentDisplayTime = 0;
	currentFrame = 0;
	paused = true;
	texture: Texture;
	animationSubject!: Mesh<any, any> | Sprite;

	constructor(
		public columns: number,
		public rows: number,
		public sprite:  Mesh<any, any> | Sprite
	) {
		this.texture = sprite.material.map!
		this.animationSubject = sprite;
	}
	...
	public setFrame(currentFrame: number): void {
		this.paused = true ;
	...
	public getRow() {
		return Math.floor(this.currentFrame / this.rows);
	};

	public getColumn() {
		return this.currentFrame % this.columns;
	};

	public offsetTexture() {
		this.texture.offset.x = this.getColumn() / this.columns;
	};
