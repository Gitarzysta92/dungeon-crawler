...
export class EffectPayloadProviderService implements IEffectPayloadProvider {
  ...
  public async collectFieldTypeData(dataType: ICollectableData, effect: IEffect): Promise<IEffectPayloadProviderResult<IField>> {
    const acceptanceProvider = provider => this._uiInteractionService.requireActivityConfirmationOrAbandon(effect.id, provider)
  ...
  public async collectRotationTypeData(dataType: ICollectableData & ICollectedDataStep, effect: IEffect): Promise<IEffectPayloadProviderResult<IBoardObjectRotation>> {
