...
export class UseAbilityCommand implements IMixinFactory<any> {
  ...
  create(e: Constructor<IUseAbilityActivity>): Constructor<ICommand> {
    ...
    class UseAbilityCommand extends e implements ICommand {
      ...
      public async execute(
        ...
        try {
          for await (let execution of this.doActivity(pawn, controller)) {
            ...
            await this._handleExecution(execution as IProcedureExecutionStatus<IGatheringDataProcedureStepDeclaration & IMakeActionProcedureStepDeclaration>);
          }
        } catch (e) {
          abandonTransaction()
          throw e;
      ...
      private async _handleExecution(execution: IProcedureExecutionStatus<IGatheringDataProcedureStepDeclaration & IMakeActionProcedureStepDeclaration>) {
