...
import { ICardsDeck, ICard } from "./cards-deck.interface";


export class CardsDeckActor implements ICardsDeck {
  id: string;
  ...
  get allCards(): ICard<IEffect>[] { 
    return [...this.cardsToUtilize, ...this.utilizedCards, ...this.cardsInDeck]
  }


  actorType: ActorType.CardsDeck = ActorType.CardsDeck;
  groupId: string;
  drawPerTurn: number;
  lastingEffects: IEffect[];
  sourceActorId: string;

  constructor(data: Omit<ICardsDeck, 'actorType'>) {
    this.id = data.id;
  ...
  public addCard(card: ICard<IEffect>, stackType: CardsDeckStackType, cardPosition: CardsDeckCardPosition): void {
  ...
  public removeCard(card: ICard<IEffect>): void {
  ...
  public revealCard(card: ICard<IEffect>): void {
  ...
  public moveCard(card: ICard<IEffect>, stackType: CardsDeckStackType, cardPosition: CardsDeckCardPosition): void {
  ...
  public addCardToUtilized(utilizedCard: ICard<IEffect>): void {
  ...
  public isInStack(card: ICard<IEffect>, stackType: CardsDeckStackType): boolean {
    const stack = this._selectStack(stackType);
    return stack.some(s => s.id === card.id);
  }

  private _selectStack(stack: CardsDeckStackType): ICard<IEffect>[] {
