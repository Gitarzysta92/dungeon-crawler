import { Dictionary, Guid } from "../../extensions/types";
import { BoardField } from "./board-field";
import { BoardHelper } from "./board.helper";
import { IBoardCoordinates, IAassignedBoardObject, IBoardSelector, IField, IBoardState, IBoardObjectRotation, IBoardSelectorOrigin, IBoardObject, IBoardSelectorDeterminant, IVectorAndDistanceEntry } from "./board.interface";
import { CoordsHelper } from "./coords.helper";
import { RotationHelper } from "./rotation.helper";

export class BoardStateHandler<F extends BoardField, O extends IAassignedBoardObject> implements IBoardState<F, O> {

  public fields: Dictionary<`${IBoardCoordinates['r']}${IBoardCoordinates['q']}${IBoardCoordinates['s']}`, F>;
  public objects: Dictionary<`${IBoardCoordinates['r']}${IBoardCoordinates['q']}${IBoardCoordinates['s']}`, O>;

  public get objectList() { return Object.values(this.objects) }

  constructor(data: IBoardState<F, O>) {
  ...
  public getObjectsAsArray<T>(): (O & T & IAassignedBoardObject)[] {
    return Object.values(this.objects) as (O & T & IAassignedBoardObject)[];
  }
