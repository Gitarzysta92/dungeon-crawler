...
export const makeAttack = (payload: {
  ...
  (state: DungeonState, feed: IGameFeed) => {
    ...
    if (payload.weaponId && payload.attack.effectName === EffectName.DealDamageByWeapon) {
      ...
      const actualTargets = getActualTargets<IEnemy & IBoardObject>(state, weapon, payload.targets);
      ...
      const actualTargets = getActualTargets<IEnemy & IBoardObject>(state, payload.attack, payload.targets);
      if (payload.targets.length > actualTargets.length) {
        throw new Error("Not all selected targets are available to take an attack");
      }
...
function getActualTargets<T extends IBoardObject>(state: DungeonState, selector: IBoardSelector, targets: T[]) {
