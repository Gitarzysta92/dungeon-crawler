import { IInteraction, IInteractionHandler } from "../../../../cross-cutting/interaction/interaction.interface";
import { IEffect, IEffectCaster } from "../../effect.interface";


export const CAST_EFFECT_INTERACTION_IDENTIFIER = "CAST_EFFECT_INTERACTION_IDENTIFIER"

export interface ICastInteraction extends IInteraction {
  id: typeof CAST_EFFECT_INTERACTION_IDENTIFIER,
}

export class CastInteractionHandler implements IInteractionHandler<ICastInteraction> {
  interactionId: string = typeof CAST_EFFECT_INTERACTION_IDENTIFIER;

  public resolveInteraction(initiator: IEffectCaster, subject: IEffect, interaction: ICastInteraction): boolean {
    return true;
  }
}