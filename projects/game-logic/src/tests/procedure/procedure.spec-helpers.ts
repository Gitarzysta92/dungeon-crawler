...
import { IProcedure, IProcedureContext, IProcedurePerformer, IProcedureStepDeclaration } from "../../lib/base/procedure/procedure.interface";
...
export class ProcedureTestStepMock extends ProcedureStep {
  ...
  perform = jest.fn<Promise<boolean>, [ProcedureAggregate, IProcedureContext, boolean]>()
    .mockImplementation(async (a, ctx, allowEarlyResolve) => {
      if (allowEarlyResolve && this.earlyResolveWhenPossible && a.passes.length > 0) {
        return await !ctx.performer.listenForEarlyResolve(true);
      }

      ctx = Object.assign(a.createExecutionContext(this), ctx);
      if (JsonPathResolver.isResolvableReference(this.executionsNumber)) {
        this.executionsNumber = JsonPathResolver.resolveInline(this.procedure as ResolvableReference<IProcedure>, ctx)
      }
